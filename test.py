...
class TargetTest(AbstractTrainer):
   ...
   This class contain the main training functions for our AdAtime
    ...
    def model_test(self, chkpoint):
        ...
        self.algorithm.network.load_state_dict(chkpoint, strict=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--da_method', default='SRR', type=str, help='')

    # ========= Select the DATASET ==============
    parser.add_argument('--data_path', default=r'D:/Dataset', type=str, help='Path containing dataset')
    parser.add_argument('--dataset', default='HAR', type=str, help='Datasets')

    # ========= Select the BACKBONE ==============
    parser.add_argument('--backbone', default='TemporalSpatialNN_new', type=str, help='Backbone Setting.')

    # ========= Experiment settings ===============
    parser.add_argument('--num_runs', default=None, type=int, help='Number of consecutive run with different seeds')
    parser.add_argument('--device', default="cuda", type=str, help='cpu or cuda')
    parser.add_argument('--gpu_id', default=0, type=str, help='gpu id.')

    args = parser.parse_args()
    dataset_class = get_dataset_class(args.dataset)
    dataset_cfg = dataset_class()

    if hasattr(dataset_cfg, "num_runs"):
        args.num_runs = dataset_cfg.num_runs
    else:
        args.num_runs = len(dataset_cfg.scenarios)

    print(f"[INFO] num_runs is set to {args.num_runs} for dataset {args.dataset}")
